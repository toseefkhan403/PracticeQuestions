...
public class Subset2 {
    ...
    // add to res at every step - loop thru ahead - if there are duplicates, only pick for the first time
    // O(2^n*n), O(2^n*k)
    ...
    public void subsetRecurBrute(int[] nums, int index, List<Integer> ds, List<List<Integer>> result) {
        ...
        ds.add(nums[index]);
        subsetRecurBrute(nums, index + 1, ds, result);
