...
public class LongestSubarraySumK {
    ...
    // calc all subarrays - keep max for sum 0 - O(n^2) - todo rewrite
    ...
    // if >k remove and inc i till >k - todo rewrite
    ...
    // OPTIMAL IF NEGATIVE NUMBERS EXIST
    ...
    public static int findMaxLength(int[] nums, int sum) {
        ...
        for (int i = 0; i < nums.length; i++) {
            ...
            // else map values get overriden(we need longest)
            else {
                map.put(currSum, i);
            }
        }
